...
const ownFilter = (user) =>
  hasPermission(user, "quotations_view_all")
    ? {}
    : { $or: [{ createdBy: user.id }, { assignedTo: user.id }] };
const owns = (user, quote) =>
  [quote.createdBy, quote.assignedTo].some(
    (value) => value?.toString() === user.id,
  );
const canEdit = (user, quote) =>
  hasPermission(user, "quotations_manage_all") || owns(user, quote);
const normalizeQuotationBody = (source) => ({
  ...source,
...
function validateCommercialTerms(body) {
  ...
  const schedule = Array.isArray(body.paymentSchedule)
  ...
  if (schedule.length) {
    ...
    if (
      schedule.some((milestone) => !String(milestone.label || "").trim()) ||
      Math.abs(total - 100) > 0.01
    )
      return "Payment milestones must have labels and total exactly 100%";
  }
  if (
    body.prosumerIncluded &&
    Number(body.sanctionedLoad || 0) > 0 &&
    Number(body.systemSize || 0) > Number(body.sanctionedLoad)
  )
...
function calculateTotals(source) {
  const items = (source.items || [])
    .filter(
      (item) => Number(item.quantity) > 0 && String(item.name || "").trim(),
    )
    .map((item) => ({
      ...item,
  ...
  const discountAmount =
    source.discountType === "fixed" ? discount : subtotal * (discount / 100);
...
async function customerAllowed(user, customerId) {
  const access = hasPermission(user, "customers_view_all")
    ? {}
    : { $or: [{ createdBy: user.id }, { assignedTo: user.id }] };
...
router.get("/", async (req, res) => {
  ...
  const sort =
    req.query.sort === "amount_desc"
      ? { grandTotal: -1 }
      : req.query.sort === "amount_asc"
        ? { grandTotal: 1 }
        : { createdAt: -1 };
  if (!req.query.page && Object.keys(req.query).length === 0)
    return res.json(await Quotation.find(filter).sort(sort));
  const { page, limit, skip } = pagination(req.query);
  const [items, total] = await Promise.all([
    Quotation.find(filter).sort(sort).skip(skip).limit(limit).lean(),
    Quotation.countDocuments(filter),
  ]);
  res.json(
    paginated(
      items.map((item) => ({ ...item, id: item._id.toString() })),
      total,
      page,
      limit,
    ),
...
router.get("/export/csv", async (req, res) => {
  if (!hasPermission(req.user, "exports_manage"))
    return fail(res, 403, "FORBIDDEN", "Export access is required");
  const rows = await Quotation.find(ownFilter(req.user))
    .sort({ createdAt: -1 })
    .limit(5000)
    .lean();
  const escape = (value) => `"${String(value ?? "").replaceAll('"', '""')}"`;
  const csv = [
    "Reference,Status,System kW,Total,Created",
    ...rows.map((row) =>
      [
        row.quotationNumber,
        row.status,
  ...
  await writeAudit(req, {
    ...
    summary: `Exported ${rows.length} quotations`,
  });
...
router.get("/:id", async (req, res) => {
  ...
  if (!quotation)
    return fail(res, 404, "QUOTATION_NOT_FOUND", "Quotation not found");
  res.json(quotation);
});
...
router.post("/", async (req, res) => {
  ...
  if (body.templateId && !isObjectId(body.templateId))
    return fail(
      ...
      400,
      "TEMPLATE_INVALID",
      "Select a valid quotation template",
    );
  if (!(await customerAllowed(req.user, body.customerId)))
    return fail(
      res,
      400,
      "CUSTOMER_INVALID",
      "Select a customer assigned to you",
    );
  const commercialError = validateCommercialTerms(body);
  if (commercialError)
    return fail(res, 400, "COMMERCIAL_INVALID", commercialError);
  const totals = calculateTotals(body);
  if (!totals.items.length || totals.subtotal <= 0)
    return fail(
      res,
      400,
      "ITEMS_REQUIRED",
      "Add at least one priced line item before saving",
    );
  const discount = Number(body.discount || 0);
  if (body.discountType === "percent" && discount > 100)
    return fail(
      res,
      400,
      "DISCOUNT_INVALID",
  ...
  const status = ["draft", "sent"].includes(body.status)
    ? body.status
    : "draft";
  const {
    ...
    commercialSnapshot: _snapshot,
    approval: _approval,
    revision: _revision,
    revisionOf: _revisionOf,
    ...input
  } = body;
  const quotation = await Quotation.create({
    ...
    status,
    assignedTo:
      hasPermission(req.user, "quotations_manage_all") && body.assignedTo
        ? body.assignedTo
        : req.user.id,
  ...
  await writeAudit(req, {
    ...
    summary: `Created quotation ${quotation.quotationNumber}`,
  });
...
router.put("/:id", async (req, res) => {
  ...
  if (!quotation)
    return fail(res, 404, "QUOTATION_NOT_FOUND", "Quotation not found");
  if (!canEdit(req.user, quotation))
    return fail(
      res,
      403,
      "FORBIDDEN",
      "You can update quotations assigned to you only",
    );
  ...
  if (body.customerId && !(await customerAllowed(req.user, body.customerId)))
    return fail(
      res,
      400,
      "CUSTOMER_INVALID",
  ...
  if (!hasPermission(req.user, "quotations_manage_all"))
    delete updates.assignedTo;
  if (
    updates.status &&
    updates.status !== previousStatus &&
    !transitions[previousStatus]?.includes(updates.status)
  )
    return fail(
      res,
      409,
      "STATUS_TRANSITION_INVALID",
      `Cannot move a ${previousStatus} quotation to ${updates.status}`,
    );
  if (updates.status && !["draft", "sent"].includes(updates.status))
    return fail(
      res,
      409,
      "STATUS_ENDPOINT_REQUIRED",
      "Use the approval or status action for this quotation state",
    );
  ...
  await writeAudit(req, {
    ...
    entityType: "quotation",
    entityId: quotation.id,
    summary: `Updated quotation ${quotation.quotationNumber}`,
  });
  res.json(quotation);
});
...
router.post("/:id/duplicate", async (req, res) => {
  ...
  const reference =
    ...
    `${original.quotationNumber}-COPY-${Date.now().toString().slice(-5)}`;
  const copy = await Quotation.create({
    ...
    updatedBy: req.user.id,
    approval: {},
    statusHistory: [{ status: "draft", changedBy: req.user.id }],
  });
  await writeAudit(req, {
    action: "quotation.duplicated",
    entityType: "quotation",
    entityId: copy.id,
    summary: `Duplicated ${original.quotationNumber} as ${reference}`,
  });
...
router.post("/:id/revisions", async (req, res) => {
  ...
  const latest = await Quotation.findOne({
    $or: [{ _id: rootId }, { revisionOf: rootId }],
  })
    .sort({ revision: -1 })
    .lean();
  delete original._id;
  ...
  const reference =
    req.body.quotationNumber ||
    `${original.quotationNumber.replace(/-R\d+$/, "")}-R${revision}`;
  ...
  await writeAudit(req, {
    ...
    entityType: "quotation",
    entityId: created.id,
    summary: `Created revision ${revision} of ${original.quotationNumber}`,
  });
...
router.post("/:id/approval", async (req, res) => {
  ...
  if (!canEdit(req.user, quotation))
    return fail(
      ...
      403,
      "FORBIDDEN",
      "You cannot request approval for this quotation",
    );
  ...
  await quotation.save();
  await writeAudit(req, {
    action: "quotation.approval_requested",
    entityType: "quotation",
    entityId: quotation.id,
    summary: `Approval requested for ${quotation.quotationNumber}`,
  });
  res.json(quotation);
});

router.patch("/:id/approval", async (req, res) => {
  if (!hasPermission(req.user, "quotations_approve"))
    return fail(res, 403, "FORBIDDEN", "Quotation approval access is required");
  const quotation = await Quotation.findById(req.params.id);
  if (!quotation)
    return fail(res, 404, "QUOTATION_NOT_FOUND", "Quotation not found");
  ...
  quotation.approval.decidedBy = req.user.id;
  quotation.approval.decidedAt = new Date();
  quotation.approval.note = String(req.body.note || "");
  quotation.statusHistory.push({
    status: quotation.status,
    changedBy: req.user.id,
    note: req.body.note || "",
  ...
  await quotation.save();
  await writeAudit(req, {
    action: `quotation.${quotation.status}`,
    entityType: "quotation",
    entityId: quotation.id,
    summary: `${approved ? "Approved" : "Rejected"} ${quotation.quotationNumber}`,
  });
  res.json(quotation);
});
...
router.patch("/:id/status", async (req, res) => {
  ...
  if (!quotation)
    return fail(res, 404, "QUOTATION_NOT_FOUND", "Quotation not found");
  if (!canEdit(req.user, quotation))
    return fail(res, 403, "FORBIDDEN", "You cannot change this quotation");
  ...
  if (
    ...
    return fail(
      res,
      409,
      "APPROVAL_ENDPOINT_REQUIRED",
      "Use the quotation approval action for this status change",
    );
  if (
    !statuses.includes(next) ||
    !transitions[quotation.status]?.includes(next)
  )
    return fail(
      res,
      409,
      "STATUS_TRANSITION_INVALID",
      `Cannot move a ${quotation.status} quotation to ${next}`,
    );
  ...
  await writeAudit(req, {
    ...
    entityType: "quotation",
    entityId: quotation.id,
    summary: `${quotation.quotationNumber} moved to ${next}`,
  });
  res.json(quotation);
});
...
router.delete("/:id", async (req, res) => {
  ...
  if (!quotation)
    return fail(res, 404, "QUOTATION_NOT_FOUND", "Quotation not found");
  if (
    !hasPermission(req.user, "quotations_delete_all") &&
    !(
      owns(req.user, quotation) &&
      hasPermission(req.user, "quotations_delete_own")
    )
  )
    return fail(res, 403, "FORBIDDEN", "You cannot delete this quotation");
  await quotation.deleteOne();
  await writeAudit(req, {
    action: "quotation.deleted",
    entityType: "quotation",
    entityId: quotation.id,
    summary: `Deleted ${quotation.quotationNumber}`,
  });
