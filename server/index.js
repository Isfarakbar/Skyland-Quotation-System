...
app.use("/api", async (req, res, next) => {
  ...
  if (req.path === "/auth/me" && !req.cookies?.[AUTH_COOKIE] && !hasBearerToken)
    return next();
  try {
    initializationPromise ||= connectDB().then(() =>
      process.env.NODE_ENV === "test" || process.env.BOOTSTRAP_ON_START === "1"
        ? seedMongoDB()
        : undefined,
    );
...
app.use((error, req, res, _next) => {
  ...
  const message =
    process.env.NODE_ENV === "production"
      ? safeMessage(error)
      : error.message || safeMessage(error);
  const code =
    error?.code === 11000
      ? "CONFLICT"
