...
import { getInitials, formatDate, matchesSearch, debounce, formatWhatsAppNumber } from '../utils/helpers.js';
      ...
      <div class="page-toolbar">
        <div class="page-toolbar-left">
          <div class="search-input-wrapper">
            ${createIcon('search')}
            <input type="text" class="search-input" placeholder="Search customers..." id="customer-search" value="${searchQuery}" />
          </div>
        </div>
      </div>
              ...
              <tr>
                <td>
                  <div class="customer-name-cell">
                    ...
                    <div>
                      ...
                      ${c.email ? `<div class="text-xs text-secondary">${c.email}</div>` : ''}
                    </div>
                  </div>
                </td>
                <td>${c.phone || '-'}</td>
                <td>${c.city || '-'}</td>
                <td><span class="badge badge-category">${c.projectType || 'N/A'}</span></td>
                <td>${quoteCount}</td>
                ...
                <td>
                  <div class="table-actions">
                    ...
                    <button class="btn btn-ghost btn-icon btn-sm" data-action="delete" data-id="${c.id}" data-tooltip="Delete" style="color: var(--color-danger-light);">
                      ${createIcon('trash')}
                    </button>
                  </div>
                </td>
...
function bindTableActions(container, customers) {
  container.querySelectorAll('[data-action="edit"]').forEach(btn => {
    btn.addEventListener('click', () => {
      const customer = customers.find(c => c.id === parseInt(btn.dataset.id));
  ...
  container.querySelectorAll('[data-action="delete"]').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      if (confirmed) {
        await deleteCustomer(parseInt(btn.dataset.id));
  ...
  container.querySelectorAll('[data-action="quote"]').forEach(btn => {
    btn.addEventListener('click', () => {
      navigate('/quotation-builder/' + btn.dataset.id);
    });
  });
}
    ...
    <form id="customer-form" autocomplete="off">
      <div style="display: flex; flex-direction: column; gap: 1rem;">
        <div class="form-group">
          ...
          <input type="text" class="form-input" id="cust-name" value="${c.name || ''}" placeholder="e.g., Mr. Abid Ali" required />
        </div>

        <div class="form-row">
          <div class="form-group">
            <label class="form-label">Phone Number <span class="required">*</span></label>
            <input type="tel" class="form-input" id="cust-phone" value="${c.phone || ''}" placeholder="e.g., 0300-1234567" required />
          </div>
          <div class="form-group">
            <label class="form-label">WhatsApp Number</label>
            <input type="tel" class="form-input" id="cust-whatsapp" value="${c.whatsapp || ''}" placeholder="Same as phone if blank" />
        ...
        <div class="form-group">
          ...
          <input type="email" class="form-input" id="cust-email" value="${c.email || ''}" placeholder="email@example.com" />
        </div>

        <div class="form-row">
          <div class="form-group">
            <label class="form-label">City <span class="required">*</span></label>
            <input type="text" class="form-input" id="cust-city" value="${c.city || ''}" placeholder="e.g., Lahore" required />
          </div>
          <div class="form-group">
        ...
        <div class="form-group">
          ...
          <textarea class="form-textarea" id="cust-address" placeholder="Full address" style="min-height: 60px;">${c.address || ''}</textarea>
        </div>

        <div class="form-group">
          <label class="form-label">Notes</label>
          <textarea class="form-textarea" id="cust-notes" placeholder="Internal notes..." style="min-height: 60px;">${c.notes || ''}</textarea>
        </div>
      </div>
